...
import { ShieldCheck, LogOut, Users, ChevronRight, BookOpen, Pencil, LayoutDashboard, GraduationCap, TrendingUp, Trash2, Search  } from 'lucide-react'
...
function AdminPanel({ onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-[#07060f] flex">
      ...
      <div className="w-64 bg-[#0f0d1a] border-r border-white/5 flex flex-col p-6 gap-6 shrink-0">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <motion.div
                  ...
                  className="bg-[#0f0d1a] border border-white/5 rounded-2xl p-6 flex items-center gap-4 hover:border-purple-500/20 transition-colors"
                >
            ...
            <div className="bg-[#0f0d1a] border border-white/5 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                <h2 className="text-white font-black text-base">Últimos Alunos Cadastrados</h2>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-all cursor-pointer"
              >
                + Novo Usuário
              </button>
            </div>
            ...
            {loading ? (
              ...
              <div className="bg-[#0f0d1a] border border-white/5 rounded-2xl p-10 flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-white font-bold">Nenhum usuário cadastrado</p>
                <p className="text-gray-500 text-sm">Crie o primeiro usuário para começar.</p>
          ...
          >
            ...
            {classes.length === 0 ? (
              <div className="bg-[#0f0d1a] border border-white/5 rounded-2xl p-10 flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-gray-500 text-sm">Crie a primeira turma para começar.</p>
                <button onClick={() => setShowNewClass(true)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-all cursor-pointer">
                  + Nova Turma
                </button>
              </div>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-8 h-8 bg-purple-600/20 hover:bg-purple-600/40 border border-purple-500/20 hover:border-purple-500/40 text-purple-400 rounded-lg flex items-center justify-center transition-all cursor-pointer shrink-0"
                    >
                      <Pencil size={14} />
                    </button>
                    <button
                      onClick={() => handleDeleteClass(cls.id)}
                      className="w-8 h-8 bg-red-600/20 hover:bg-red-600/40 border border-red-500/20 hover:border-red-500/40 text-red-400 rounded-lg flex items-center justify-center transition-all cursor-pointer shrink-0"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
      ...
      {showNewUser && (
        <ModalNewUser onClose={() => setShowNewUser(false)} onSave={handleSaveUser} classes={classes} />
      )}

      {showNewClass && (
        <ModalNewClass onClose={() => setShowNewClass(false)} onSave={handleSaveClass} />
      )}

      {editingUser && (
        <ModalEditUser user={editingUser} classes={classes} onClose={() => setEditingUser(null)} onSave={handleUpdateUser} />
      )}

      {editingClass && (
      <ModalEditClass cls={editingClass} onClose={() => setEditingClass(null)} onSave={handleUpdateClass}
      />
    )}
